...
function transformNavigation(navigation, pages, config, language) {
    const getPageData = (path) => {
        // 如果开启了国际化且不是默认语言，先尝试获取带语言后缀的页面数据
        if (isFeatureEnabled(config, 'i18n') && language !== config.i18n?.default) {
            const langPath = `${path}.${language}`;
            const langPageData = pages.get(langPath);
            if (langPageData) {
                return langPageData;
            }
        }
        // 如果没有找到带语言后缀的数据，返回默认路径的数据
