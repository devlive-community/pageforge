...
class FileProcessor {
    ...
    async getGitFileInfo(filePath) {
        // 如果 revision 功能未启用，则返回 null
        if (!this.config.feature?.revision?.enable) {
            return null;
        }

        try {
            // 使用 --follow 来跟踪文件重命名历史，确保获取到真正的第一次提交
            ...
            // 只有在启用了 contributors 功能时才获取贡献者信息
            let contributors = [];
            ...
            if (this.config.feature?.contributors?.enable) {
                ...
                contributors = execSync(contributorsCommand, { encoding: 'utf-8' })
                    .trim()
                    .split('\n')
                    .filter(Boolean);
                ...
                contributionStats = execSync(contributionStatsCommand, { encoding: 'utf-8' })
                    .trim()
                    .split('\n')
                    .filter(Boolean)
                    .map(line => {
            ...
            // 只有在启用了 contributors 功能时才添加贡献者信息
            if (this.config.feature?.contributors?.enable) {
                result.contributors = contributors;
                result.contributionStats = contributionStats;
            }
